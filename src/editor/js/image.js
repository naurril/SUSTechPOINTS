    ...
    if (calib) {
      ...
      if (img && (img.naturalWidth > 0)) {
        ...
        if (imgfinal != null) { // if projection is out of range of the image, stop drawing.
          ...
          const crop_area = crop_image(img.naturalWidth, img.naturalHeight, ctx.canvas.width, ctx.canvas.height, imgfinal)
...
class ImageContext extends ResizableMoveableView {
  ...
  generate2dRects () {
    ...
    return this.world.annotation.boxes.map((box) => {
      ...
      const ptsTopPartOnImg = points3d_to_image2d(points3dTopPart, calib, true, null, img.width, img.height)
      const ptsGroundPartOnImg = points3d_to_image2d(points3dGroundPart, calib, true, null, img.width, img.height)
  ...
  generate2dRectById (id) {
    ...
    const box = this.world.annotation.boxes.find(b => b.obj_track_id == id)
  ...
  draw_svg () {
    ...
    if (this.cfg.projectLidarToImage) {
      ...
      const lidar_pts = this.world.lidar.get_all_points()
      const lidar_pts_color = this.world.lidar.get_all_colors()

      const img_lidar_point_map = []
      const ptsOnImg = points3d_to_image2d(lidar_pts, calib, true, img_lidar_point_map, img.width, img.height)
      ...
      if (this.cfg.color_points === 'intensity') {
        ...
        for (let i = 0; i < ptsOnImg.length / 2; i++) {
          ...
          const lidar_point_index = img_lidar_point_map[y * this.img.width + x][0]
          const intensity = lidar_pts_color[lidar_point_index * 3]
      ...
      // there may be none after projecting
      if (ptsOnImg && ptsOnImg.length > 0) {
        const pts_svg = this.points_to_svg(ptsOnImg, trans_ratio, 'svg-points-lidar' /* css */, 1 /* size */, img_pts_color)
        svg.appendChild(pts_svg)
      }
    }
  }
  ...
  box_to_svg (box, box_corners, trans_ratio, selected, svg) {
    ...
    const right_pannel = create_panel(pts, 'nofill')
    ...
    const top_pannel = create_panel(pts, 'nofill')
  ...
  boxes_manager = {
    ...
    add_box: (box) => {
      ...
      if (trans_ratio) {
        ...
        if (imgfinal) {
          ...
          const svg_box = this.box_to_svg(box, imgfinal, trans_ratio)
          const svg = this.ui.querySelector('#svg-boxes')
          if (svg_box) { svg.appendChild(svg_box) }
        }
      }
    },
    ...
    onBoxUnselected: (box_obj_local_id, obj_type) => {
      ...
      if (b) { b.setAttribute('class', obj_type + ' svg-box') }
    },
    ...
    update_box: (box) => {
      const b = this.ui.querySelector('#svg-box-local-' + box.obj_local_id)
      if (!b) {
        return
      }
...
class ImageContextManager {
  ...
  addImage (name, autoSwitch) {
    ...
    const image = new ImageContext(this.parentUi, this, name, autoSwitch, this.cfg, this.on_img_click)
  ...
  setBestCamera (camera) {
    this.images.filter(i => i.autoSwitch).forEach(i => {
      ...
      if (changedCamera) { i.boxes_manager.display_image() }
    })
...
function points3d_homo_to_image2d (points3d, calib, accept_partial = false, save_map, img_dx, img_dy) {
  ...
  const pos_in_camera_space = imgpos
...
function choose_best_camera_for_point (world, center) {
  ...
  const valid_proj_pos = proj_pos.filter(function (p) {

...
def placeholder_inputs(batch_size, num_point, regression=False):
...
def get_model(point_cloud, is_training, num_classes, bn_decay=None, regression=False):
    ...
    if regression:
        ...
        net = tf_util.fully_connected(net, num_classes, activation_fn=None, scope='fc3')  # so 40 means there are 40 classes.

    return net, end_points


def get_loss(pred, label, end_points, reg_weight=0.001, regression=False):  #pred is net returned in  `get_model`
    """ pred: B*NUM_CLASSES,
        label: B, """
